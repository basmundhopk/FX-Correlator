...
void unpack(
    ...
    hls::stream<std::complex<data_t>> &ch0,
    hls::stream<std::complex<data_t>> &ch1,
    hls::stream<std::complex<data_t>> &ch2,
    hls::stream<std::complex<data_t>> &ch3
) {
    #pragma HLS INLINE off
    ...
    unpack_loop: for (int i = 0; i < FFT_LEN; i++) {
        ...
        ch0.write(std::complex<data_t>(pack.ch[0].i, pack.ch[0].q));
        ch1.write(std::complex<data_t>(pack.ch[1].i, pack.ch[1].q));
        ch2.write(std::complex<data_t>(pack.ch[2].i, pack.ch[2].q));
        ch3.write(std::complex<data_t>(pack.ch[3].i, pack.ch[3].q));
    }
}
...
void repack(
    ...
    repack_loop: for (int i = 0; i < FFT_LEN; i++) {
        ...
        std::complex<data_t> val0 = ch0.read();
        std::complex<data_t> val1 = ch1.read();
        std::complex<data_t> val2 = ch2.read();
        std::complex<data_t> val3 = ch3.read();
