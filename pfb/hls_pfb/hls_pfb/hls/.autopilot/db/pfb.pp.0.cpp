# 1 "/home/brett/Documents/FX_Correlator/pfb/hls_pfb/pfb.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "/home/brett/Documents/FX_Correlator/pfb/hls_pfb/pfb.cpp" 2
# 1 "./pfb.h" 1
...
__attribute__((sdx_kernel("pfb_multichannel_decimator", 0))) void pfb_multichannel_decimator(
    ...
    const coeff_t coeff[(1024 * 4)]
);
# 2 "/home/brett/Documents/FX_Correlator/pfb/hls_pfb/pfb.cpp" 2
...
#pragma HLS INLINE off
 ...
 read_loop: for (int i = 0; i < 1024; i++) {
...
#pragma HLS INTERFACE ap_ctrl_hs port=return
 ...
 static iq_sample_t branch_memory[4][4][1024];
...
#pragma HLS BIND_STORAGE variable=branch_memory type=RAM_1P impl=BRAM
 ...
 static ap_uint<2> write_bank_idx = 0;
    ...
    static coeff_t local_coeffs[(1024 * 4)];
    static bool coeff_loaded = false;

    if (!coeff_loaded) {
        load_coeffs: for(int i=0; i<(1024 * 4); i++) {
#pragma HLS PIPELINE II=1 rewind=false
 local_coeffs[i] = coeff[i];
    ...
    compute_loop: for (int k = 0; k < 1024; k++) {
#pragma HLS PIPELINE II=1 rewind=false

 parallel_sample_t input_pack = internal_in.read();

        store_loop: for(int c=0; c<4; c++) {
#pragma HLS UNROLL
...
#pragma HLS INTERFACE ap_ctrl_hs port=return
 ...
 data_32 packed_val;
    ...
    write_loop: for (int i = 0; i < 1024; i++) {
#pragma HLS PIPELINE II=1 rewind=false
...
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=pfb_multichannel_decimator
# 140 "/home/brett/Documents/FX_Correlator/pfb/hls_pfb/pfb.cpp"
