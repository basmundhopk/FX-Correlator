...
//------------------------Parameter----------------------
`define TV_IN_gmem "../tv/cdatafile/c.pfb_multichannel_decimator.autotvin_gmem.dat"
`define TV_OUT_gmem "../tv/rtldatafile/rtl.pfb_multichannel_decimator.autotvout_gmem.dat"
...
parameter gmem_transaction_depth = 32'd 4096;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("/home/brett/Documents/FX_Correlator/pfb/hls_pfb/hls_pfb/hls/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("/home/brett/Documents/FX_Correlator/pfb/hls_pfb/hls_pfb/hls/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < gmem_transaction_depth; i = i + 1) begin 
          ...
          if (i%2 == 1) begin
              case(mem_page)
                  0 : gmem_mem_0[i/2] = mem_tmp;
                  1 : gmem_mem_1[i/2] = mem_tmp;
                  2 : gmem_mem_2[i/2] = mem_tmp;
                  3 : gmem_mem_3[i/2] = mem_tmp;
